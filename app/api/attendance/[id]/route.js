...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    if (!student) {
      return new Response(JSON.stringify({ message: 'Attendance record not found' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' },
      });
    }
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: 'Attendance deleted successfully' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
