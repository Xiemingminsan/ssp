...
export async function GET(req, { params }) {
  ...
  try {
    const student = await Student.findById(id).populate('batch').populate('courses').populate('attendance.course');
    if (!student) {
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: 'Student updated successfully', student: updatedStudent }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
